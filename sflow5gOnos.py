...
def setup_sflow(net, collector_ip='127.0.0.1', sampling_rate='10', polling_interval='10'):
    """
    Setup sFlow configuration for all switches and access points in the network.
    """
    for device in net.switches + net.aps:
        for intf in device.intfList():
            if 'eth' in intf.name:
               sflow_cmd = 'ovs-vsctl -- --id=@sflow create sflow agent=%s target=%s sampling=%s polling=%s -- ' \
                           ' -- set bridge %s sflow=@sflow' % (intf.name, collector_ip, sampling_rate, polling_interval, device.name)
               device.cmd(sflow_cmd)
               info('sFlow configured on %s using interface %s\n' %(device.name, intf.name))
...
def myNetwork():
    ...
    info( '*** setting up sflow\n')
    setup_sflow(net,collector_ip='127.0.0.1')
